...
router.put('/test/:id', function (req, res, next) {
    console.log(req.body)
    const fournisseurId = req.body.params
    Fournisseur.findByIdAndUpdate( fournisseurId , { $push: { reclamtion: req.body }}, function (err, fournisseur) {
        if (err) {
            console.log(err);
    ...
    router.put('/changestatut/:id', function (req, res, next) {
        ...
        Fournisseur.updateOne(
            { _id: fournisseurId, "reclamtion._id":req.body.reclamtion._id  },
            { $set: { "reclamtion.$.statut" : req.body.reclamtion.statut } }
